...
import scrapy
import os.path
from pycrawler.items import Article
from pycrawler.loaders import VentureBeatArticleLoader
...
class VentureBeatSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        item = l.load_item()
        content = item['text']
        filename = 'doc'+str(self.count)+'.txt'
        self.count += 1
        f = open(os.path.join(os.path.abspath('.'),self.path,filename),'w+')
